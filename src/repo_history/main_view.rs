use crate::repo_history::model::{MultiRepoHistory, RepoCommit};
use crate::ui_common::styles::{GREEN, RED, WHITE};
use crate::ui_common::table_view::{TableView, TableViewItem};
...
pub struct MainView {
    layout: LinearLayout,
    commit_bar_model: Arc<Mutex<String>>,
}

impl MainView {
    pub fn from(model: MultiRepoHistory) -> Self {
        let table = Self::new_table(model);
        let commit_bar_model = Arc::new(Mutex::new(String::from("")));
    ...
    where
        F: Fn(&mut Cursive, usize, usize, &RepoCommit) + Send + Sync + 'static,
    {
